use crate::{
    ...
    util::{
        difficulty::{
            norm, reverse_lerp, smootherstep
        },
        float_ext::FloatExt
    }
};
...
impl ReadingEvaluator {
    ...
    fn calculate_density_difficulty(
        ...
        if let Some(next) = next_obj {
            future_object_difficulty_influence *= smootherstep(next.lazy_jump_dist, 15.0, Self::DISTANCE_INFLUENCE_THRESHOLD);
        }

        let mut note_density_difficulty = (past_object_difficulty_influence + future_object_difficulty_influence).powf(1.7) 
            * 0.4 
            * constant_angle_nerf_factor 
            * velocity;

        note_density_difficulty = 0.0_f64.max(note_density_difficulty - Self::DENSITY_DIFFICULTY_BASE);
    ...
    fn calculate_preempt_difficulty(
        ...
        let mut preempt_difficulty = ((Self::PREEMPT_STARTING_POINT - preempt + (preempt - Self::PREEMPT_STARTING_POINT).abs()) / 2.0).powf(2.5) 
    ...
    fn calculate_hidden_difficulty(
        ...
        let density_factor = (current_visible_object_density + past_object_difficulty_influence).powf(3.3) * 3.0;
    ...
    fn get_past_object_difficulty_influence(
        ...
        for i in 0..curr.idx {
            ...
            if time_diff > Self::READING_WINDOW_SIZE 
                || loop_obj.start_time + self.time_preempt < curr.start_time {  // * Current object not visible at the time object needs to be clicked
                break;
            }
    ...
    fn retrieve_current_visible_object_density(
        ...
        while let Some(hit_object) = curr.next(i, objects) {
            ...
            if time_diff > Self::READING_WINDOW_SIZE 
                || curr.start_time + self.time_preempt < hit_object.start_time { // * Current object not visible at the time object needs to be clicked
                break;
            }
